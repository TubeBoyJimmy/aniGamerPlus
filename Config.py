...
def invalid_cookie():
    ...
    if os.path.exists(cookie_path):
        ...
        try:
            ...
            # 退而求其次: 內容備份後原地清空, 效果等同標記失效。
            # 備份必須放在 logs/ (整個目錄有掛載出來): 寫在 cookie.txt 旁的話, 逐檔 bind mount
            # 下會落在容器可寫層, host 端看不到且 docker compose down 重建容器即蒸發
            try:
                backup_path = os.path.join(logs_dir, 'invalid_cookie.txt')
                with cookie_write_lock:
                    ...
                    if old_content.strip():
                        ...
                        with open(cookie_path, 'w', encoding='utf-8') as f:
                            f.write('')
                __color_print(0, 'cookie狀態', '已清空失效cookie (bind mount 無法改名), 內容備份於 logs/invalid_cookie.txt',
                              no_sn=True, status=1)
